...
<div class="login_page">
                    ...
                    <div class="col-xs-12 col-sm-6 address">
                        <ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if} box">
                            ...
                            <li class="address_update">
                                <button class="button btn-update" onclick="location.href='{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}'" title="{l s='Update'}"><span>{l s='Update'}</span></button>
                                <a class="button continue" href='{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}' onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}"><span>{l s='Delete'}</span></a></li>
                        </ul>
                    </div>
                {/foreach}
            </div>
    ...
    <div class="clearfix main-page-indent">
        <a href="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" title="{l s='Add an address'}" class="button continue button button-medium"><span>{l s='Add a new address'}</span></a>
    </div>
    <ul class="footer_links clearfix">
        <a class="button continue button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><span> {l s='Back to your account'}</span></a>
        <a class="button continue button-small right" href="{$base_dir}"><span> {l s='Home'}</span></a>
    </ul>
</div>
