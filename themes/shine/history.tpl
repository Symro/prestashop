...
<div class="login_page">
    ...
    <div class="block-center" id="block-history">
        {if $orders && count($orders)}
            <table class="data-table cart-table   table-bordered footab" id="order-list">
                <colgroup>
                    <col width="1">
                    <col width="1">
                    <col width="1">
                    ...
                    {foreach from=$orders item=order name=myLoop}
                        <tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
                            ...
                            <td class="history_detail">
                                <a class="button continue button button-small" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
                                    <span>
                                        {l s='Details'}</i>
                                    </span>
                                </a>
                                {if isset($opc) && $opc}
                                    <a class="button btn-update link-button" href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
                                    {else}
                                        <a class="button btn-update link-button" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
                                        {/if}
    ...
    <a class="button continue button button-small" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
        <span>
            {l s='Back to Your Account'}
        </span>
    </a>

    <a class="button continue button button-small right" href="{$base_dir}">
        <span>{l s='Home'}</span>
    </a>
