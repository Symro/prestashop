...
require (dirname(__FILE__).'/wg24menutoplinks.class.php');

class Wg24TopMenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	{
		$this->name = 'wg24topmenu';
                $this->author = '24WebGroup';
                $this->tab = 'front_office_features';
                $this->version = '1.1.0';
                $this->need_instance = 0;
		parent::__construct();
		$this->displayName = $this->l('24WebGroup Top Horizontal Menu');
                $this->confirmUninstall = $this->l('Are you sure you want to uninstall on this module?');
		$this->description = $this->l('Add a new horizontal menu to the top of your e-commerce website By 24WebGroup.');
	}
	...
	public function getContent()
	{
		...
		else if (Tools::isSubmit('submitBlocktopmenuLinks'))
		{
			...
			{
				wg24menutoplinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
		...
		else if (Tools::isSubmit('submitBlocktopmenuRemove'))
		{
			...
			wg24menutoplinks::remove($id_linksmenutop, (int)Shop::getContextShopID());
		...
		else if (Tools::isSubmit('submitBlocktopmenuEdit'))
		{
			...
			{
				$tmp = wg24menutoplinks::getLinkLang($id_linksmenutop, $id_shop);
			...
			{
				wg24menutoplinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_linksmenutop, (int)$id_linksmenutop);
		...
		$links = wg24menutoplinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = wg24menutoplinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
...
<div class="margin-form">';
		...
		$links = wg24menutoplinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = wg24menutoplinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = wg24menutoplinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'PRD':
					...
					if (!is_null($product->id))
						$this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($product->getLink()).'"><span>'.$product->name.'</span></a></li>'.PHP_EOL;
					break;
				...
				case 'CMS':
					...
					if (count($cms))
						$this->_menu .= '<li'.$selected.' ><a href="'.Tools::HtmlEntitiesUTF8($cms[0]['link']).'"><span>'.$cms[0]['meta_title'].'</span></a></li>'.PHP_EOL;
					break;
				...
				case 'CMS_CAT':
					...
					if (count($category))
					{
						$this->_menu .= '<li class="parent"><a href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'"><span>'.$category->name.'</span></a>';
				...
				case 'ALLMAN':
					$link = new Link;
					$this->_menu .= '<li class="parent"><a href="'.$link->getPageLink('manufacturer').'"><span>'.$this->l('All manufacturers').'</span></a><ul>'.PHP_EOL;
					$manufacturers = Manufacturer::getManufacturers();
					foreach ($manufacturers as $key => $manufacturer)
						$this->_menu .= '<li><a href="'.$link->getManufacturerLink((int)$manufacturer['id_manufacturer'], $manufacturer['link_rewrite']).'"><span>'.$manufacturer['name'].'</span></a></li>'.PHP_EOL;
					$this->_menu .= '</ul></li>';
					break;
				...
				case 'MAN':
					...
					if (!is_null($manufacturer->id))
					{
						...
						$link = new Link;
						$this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getManufacturerLink((int)$id, $manufacturer->link_rewrite)).'"><span>'.$manufacturer->name.'</span></a></li>'.PHP_EOL;
					}
					break;
				...
				case 'ALLSUP':
					$link = new Link;
					$this->_menu .= '<li class="parent"><a href="'.$link->getPageLink('supplier').'"><span>'.$this->l('All suppliers').'</span></a><ul>'.PHP_EOL;
					$suppliers = Supplier::getSuppliers();
					foreach ($suppliers as $key => $supplier)
						$this->_menu .= '<li><a href="'.$link->getSupplierLink((int)$supplier['id_supplier'], $supplier['link_rewrite']).'"><span>'.$supplier['name'].'</span></a></li>'.PHP_EOL;
					$this->_menu .= '</ul></li>';
					break;
				...
				case 'SUP':
					...
					{
						$link = new Link;
						$this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($link->getSupplierLink((int)$id, $supplier->link_rewrite)).'"><span>'.$supplier->name.'</span></a></li>'.PHP_EOL;
					}
					break;
				...
				case 'SHOP':
					...
					{
						$link = new Link;
						$this->_menu .= '<li'.$selected.'><a href="'.Tools::HtmlEntitiesUTF8($shop->getBaseURL()).'"><span>'.$shop->name.'</span></a></li>'.PHP_EOL;
					}
					break;
				case 'LNK':
					$link = wg24menutoplinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = wg24menutoplinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
						$this->_menu .= '<li><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</span></a></li>'.PHP_EOL;
					}
					break;
			}
		}
	...
	private function getCategory($id_category, $id_lang = false, $id_shop = false)
	{
		...
		{
                        if (count($children)) {
                        $this->_menu .= '<li ' . $selected . ' class="level-top parent">';
                        } else {
                        $this->_menu .= '<li ' . $selected . ' class="level-top">';
                        }     

			$this->_menu .= '<a href="'.Tools::HtmlEntitiesUTF8($category_link).'"><span>'.$category->name.'</span></a>';
	...
	private function getCMSMenuItems($parent, $depth = 1, $id_lang = false)
	{
		...
		if (count($categories) || count($pages))
		{
			...
			{
				$this->_menu .= '<li>';
				$this->_menu .= '<a href="#"><span>'.$category['name'].'</span></a>';
			...
			foreach ($pages as $page)
			{
				...
				$this->_menu .= '<a href="'.$links[0]['link'].'"><span>'.$cms->meta_title.'</span></a>';
				$this->_menu .= '</li>';
			}
