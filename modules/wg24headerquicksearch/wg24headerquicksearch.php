...
if (!defined('_PS_VERSION_'))
	exit;
class Wg24HeaderQuickSearch extends Module
{
	public function __construct()
	{
            $this->name = 'wg24headerquicksearch';
            ...
            $this->displayName = $this->l('24WebGroup Header Quick search');
            $this->confirmUninstall = $this->l('Are you sure you want to uninstall on this module?');
            $this->description = $this->l('Adds a quick search field to your website show page on header By 24WebGroup');
	}
...
public function hookDisplayMobileHeader($params)
	...
	public function hookTop($params)
	{
		...
		if (Tools::getValue('search_query') || !$this->isCached('blocksearch-top.tpl', $key))
                $this->calculHookCommon($params); 
                $get_id = Tools::getValue('ajaxSearch');
                $search_data = Tools::replaceAccentedChars(urldecode(Tools::getValue('q')));
		if ($get_id)
		{
                    $get_query = Search::find((int)(Tools::getValue('id_lang')), $search_data, 1, 12, 'position', 'desc', true);
                    foreach ($get_query as &$products){
                       $product_id = Product::getCover($products['id_product'] );
                       $existingimg_id = new Image($product_id['id_image']);
                       $search_petch =$existingimg_id->getExistingImgPath();
                       $links = new Link();
                       $products['product_link'] = $links->getProductLink($products['id_product'], $products['prewrite'], $products['crewrite']);
                       $products['existingimg_id'] =  $search_petch;
                    }
                 die(Tools::jsonEncode($get_query));

                }
			$this->smarty->assign(array(
				'blocksearch_type' => 'top',
				'search_query' => (string)Tools::getValue('search_query'),
                               'product_dir'=>_PS_BASE_URL_._THEME_PROD_DIR_,
				)
			);
