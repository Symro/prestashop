...
include_once(_PS_MODULE_DIR_.'wg24homecontentslider/wg24homecontentslider1.php');

class wg24HomeContentSlider extends Module
{
	private $_html = '';

	public function __construct()
	{
      $this->name = 'wg24homecontentslider';
       $this->author = '24WebGroup';
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->l('24WebGroup Home Content Slider');
         $this->confirmUninstall = $this->l('Are you sure you want to uninstall on this module?');
        $this->description = $this->l('Home Content slider module show on home page By 24WebGroup');
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 3; ++$i)
		{
			$slide = new Wg24HomeContentSlider1();
			$slide->position = $i;
			$slide->active = 1;
                            if ($i == 1):

                            $label = 'Customizable Theme';
                            $slider_dsc='
                    <h5>You can change colors of almost every element</h5>
                    <div style="overflow:hidden; text-align:left; padding-left:5px; margin-top:15px"><a href="#" class="btn-shop">Buy now</a></div>';
                            elseif ($i == 2):
                            $label = "You like this theme?";
                             $slider_dsc='
                                <h5 style="">Lorem ipsum dolor sit amet, consectetur adipisicing</h5>
                                <div style="overflow:hidden; text-align:left; padding-left:5px; margin-top:15px"><a href="#" class="btn-shop">Buy now</a></div>';
                            elseif ($i == 3):
                            $label = "Responsive Layout";
                             $slider_dsc='
                                <h5 style="">Consectetur adipisicing elit, sed do eiusmod.</h5>
                                <div style="overflow:hidden; text-align:left; padding-left:5px; margin-top:15px"><a href="#" class="btn-shop">Buy now</a></div>';
                            endif;
			foreach ($languages as $language)
			{
				$slide->title[$language['id_lang']] = $label;
				$slide->description[$language['id_lang']] = $slider_dsc;
				$slide->legend[$language['id_lang']] = $label;
				$slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=v16_wg24homecontentslider';
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homecontentslider` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homecontentslider_slides` (
			  `id_wg24homecontentslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homecontentslider_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new Wg24HomeContentSlider1($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'wg24homecontentslider`, `'._DB_PREFIX_.'wg24homecontentslider_slides`, `'._DB_PREFIX_.'wg24homecontentslider_slides_lang`;
		');
	}
	...
	private function _postProcess()
	{
		...
		{
			$slide = new Wg24HomeContentSlider1((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
		...
		{
			$slide = new Wg24HomeContentSlider1((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('wg24homecontentslider.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign(array('wg24homecontentslider_slides' => $slides));
		}
	...
	public function hookdisplayHeader($params)
	{
		...
		$this->smarty->assign('wg24homecontentslider', $slider);
	 ...
	 function hookdisplayHomeTabContent($params) 
	{
		...
		return $this->display(__FILE__, 'wg24homecontentslider.tpl', $this->getCacheId());
	}
	...
	{
		$this->_clearCache('wg24homecontentslider.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wg24homecontentslider (id_wg24homecontentslider_slides, id_shop)
			SELECT id_wg24homecontentslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'wg24homecontentslider
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'wg24homecontentslider_slides` hss, `'._DB_PREFIX_.'wg24homecontentslider` hs
			WHERE hss.`id_wg24homecontentslider_slides` = hs.`id_wg24homecontentslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_wg24homecontentslider_slides` as id_slide, hssl.`image`, hss.`position`, hss.`active`, hssl.`title`,
			hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
			FROM '._DB_PREFIX_.'wg24homecontentslider hs
			LEFT JOIN '._DB_PREFIX_.'wg24homecontentslider_slides hss ON (hs.id_wg24homecontentslider_slides = hss.id_wg24homecontentslider_slides)
			LEFT JOIN '._DB_PREFIX_.'wg24homecontentslider_slides_lang hssl ON (hss.id_wg24homecontentslider_slides = hssl.id_wg24homecontentslider_slides)
	...
	public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
	{
		...
		$results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'wg24homecontentslider_slides hss ON (hs.id_wg24homecontentslider_slides = hss.id_wg24homecontentslider_slides)
			LEFT JOIN '._DB_PREFIX_.'wg24homecontentslider_slides_lang hssl ON (hss.id_wg24homecontentslider_slides = hssl.id_wg24homecontentslider_slides)
			WHERE hs.`id_wg24homecontentslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
			($active ? ' AND hss.`active` = 1' : ' ')
		);
	...
	{
		$req = 'SELECT hs.`id_wg24homecontentslider_slides` as id_slide
				FROM `'._DB_PREFIX_.'wg24homecontentslider` hs
				WHERE hs.`id_wg24homecontentslider_slides` = '.(int)$id_slide;
