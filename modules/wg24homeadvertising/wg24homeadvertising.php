...
include_once(_PS_MODULE_DIR_.'wg24homeadvertising/wg24homeadvertising1.php');

class wg24homeadvertising extends Module
{
	private $_html = '';

	public function __construct()
	{
    $this->name = 'wg24homeadvertising';
        ...
        $this->displayName = $this->l('24WebGroup Home Offer Banner Module');
         $this->confirmUninstall = $this->l('Are you sure you want to uninstall on this module?');
        $this->description = $this->l('Home Offer Banner Module module show  home page into button section By 24WebGroup'); 
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 4; ++$i)
		{
			$slide = new wg24homeadvertising1();
			$slide->position = $i;
			$slide->active = 1;
                            if ($i == 1):
			...
			foreach ($languages as $language)
			{
				...
				$slide->url[$language['id_lang']] = 'http://www.prestashop.com/?utm_source=v16_wg24homeadvertising';
				$slide->image[$language['id_lang']] = 'offer_banner-'.$i.'.png';
			}
			$slide->add();
		}
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homeadvertising` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homeadvertising_slides` (
			  `id_wg24homeadvertising_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'wg24homeadvertising_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new wg24homeadvertising1($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'wg24homeadvertising`, `'._DB_PREFIX_.'wg24homeadvertising_slides`, `'._DB_PREFIX_.'wg24homeadvertising_slides_lang`;
		');
	}
	...
	private function _postProcess()
	{
		...
		{
			$slide = new wg24homeadvertising1((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
		...
		{
			$slide = new wg24homeadvertising1((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('wg24homeadvertising.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign(array('wg24homeadvertising_slides' => $slides));
		}
	...
	public function hookdisplayHeader($params)
	{
		...
		$this->smarty->assign('wg24homeadvertising', $slider);
	 ...
	 function hookdisplayHomeTabContent($params) 
	{
		...
		return $this->display(__FILE__, 'wg24homeadvertising.tpl', $this->getCacheId());
	}
	...
	{
		$this->_clearCache('wg24homeadvertising.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'wg24homeadvertising (id_wg24homeadvertising_slides, id_shop)
			SELECT id_wg24homeadvertising_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'wg24homeadvertising
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'wg24homeadvertising_slides` hss, `'._DB_PREFIX_.'wg24homeadvertising` hs
			WHERE hss.`id_wg24homeadvertising_slides` = hs.`id_wg24homeadvertising_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_wg24homeadvertising_slides` as id_slide, hssl.`image`, hss.`position`, hss.`active`, hssl.`title`,
			hssl.`url`, hssl.`legend`,hssl.`image`
			FROM '._DB_PREFIX_.'wg24homeadvertising hs
			LEFT JOIN '._DB_PREFIX_.'wg24homeadvertising_slides hss ON (hs.id_wg24homeadvertising_slides = hss.id_wg24homeadvertising_slides)
			LEFT JOIN '._DB_PREFIX_.'wg24homeadvertising_slides_lang hssl ON (hss.id_wg24homeadvertising_slides = hssl.id_wg24homeadvertising_slides)
	...
	public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
	{
		...
		$results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'wg24homeadvertising_slides hss ON (hs.id_wg24homeadvertising_slides = hss.id_wg24homeadvertising_slides)
			LEFT JOIN '._DB_PREFIX_.'wg24homeadvertising_slides_lang hssl ON (hss.id_wg24homeadvertising_slides = hssl.id_wg24homeadvertising_slides)
			WHERE hs.`id_wg24homeadvertising_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
			($active ? ' AND hss.`active` = 1' : ' ')
		);
	...
	{
		$req = 'SELECT hs.`id_wg24homeadvertising_slides` as id_slide
				FROM `'._DB_PREFIX_.'wg24homeadvertising` hs
				WHERE hs.`id_wg24homeadvertising_slides` = '.(int)$id_slide;
